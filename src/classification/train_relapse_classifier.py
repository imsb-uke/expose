...
@click.option("--wandb_project", default="relapse_training", help="Wandb project name")
...
@click.option("--domain_list", default="domain1", help="List of domains to use")
@click.option("--label_col", default="relapse", help="Label column for training")
...
# domain features
@click.option("--mask_domain_weights", type=bool, default=False, help="Whether to mask domain features based on domain weights")
@click.option("--domain_weights_path", default="data/domain_weights.pt", help="Path to the domain weights file")
@click.option("--topk", default=3840, help="Number of top-k features to mask based on domain weights")
@click.option("--use_largest", type=bool, default=True, help="Whether to use the largest domain weights for feature masking")
...
def main(**config): 
    ...
    model_dir = os.path.join("models_relapse", current_time)
    ...
    model = DecoderSAELinear(
        ...
        mask_domain_weights = config["mask_domain_weights"],
        domain_weights_path = config["domain_weights_path"],
        topk = config["topk"],
        use_largest = config["use_largest"],
    )
    ...
    if config["use_wandb"]:
        ...
        logger.experiment.define_metric("val_auprc", summary="max")
